...
def stratified_summary(per_query_df: pd.DataFrame, query_df: pd.DataFrame, output_dir: Path, model_name: str) -> None:
    ...
    if rows:
        pd.DataFrame(rows).to_csv(output_dir / f"stratified_{model_name}_qr_real.csv", index=False)
...
def main() -> None:
    ...
    require_columns(df, ["sample_id", "label", "qr_real_path"])
    ...
    gallery_paths = gallery_df["qr_real_path"].tolist()
    query_paths = query_df["qr_real_path"].tolist()
    ...
    for run in runs:
        ...
        if args.save_embeddings:
            torch.save(gallery_emb, outdir / f"gallery_emb_{run.model_name}_qr_real.pt")
            torch.save(query_emb, outdir / f"query_emb_{run.model_name}_qr_real.pt")
        ...
        per_query_df.to_csv(outdir / f"per_query_{run.model_name}_qr_real.csv", index=False)
    ...
    summary_df.to_csv(outdir / "summary_metrics_qr_real_chunked_8models.csv", index=False)

    if all_per_query:
        pd.concat(all_per_query, ignore_index=True).to_csv(
            outdir / "all_per_query_outputs_qr_real_chunked_8models.csv", index=False
        )
    ...
    print("\nSaved:")
    print(f"- {outdir / 'summary_metrics_qr_real_chunked_8models.csv'}")
    print(f"- {outdir / 'all_per_query_outputs_qr_real_chunked_8models.csv'}")
