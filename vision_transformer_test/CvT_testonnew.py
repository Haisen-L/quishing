...
# Train/Val Split
#########################
# Use existing dataset for training and validation (80/20)
...
train_loader = DataLoader(
    QRCodeHFDataset(train_df), batch_size=batch_size, shuffle=True, collate_fn=hf_collate
)
val_loader = DataLoader(
    QRCodeHFDataset(val_df),   batch_size=batch_size, shuffle=False, collate_fn=hf_collate
)

#########################
# Test Data Preparation
#########################
# Load new dataset for testing (generated QR codes)
new_csv = 'data/output_list_new/new_output_L.csv'
new_df = pd.read_csv(new_csv)
new_df['result'] = new_df['result'].astype(int)
# Drop entries without valid QR_Code_Path (float/None)
new_df = new_df[new_df['QR_Code_Path'].notnull()]
print(f"New test set size: {len(new_df)}")
test_loader = DataLoader(
    QRCodeHFDataset(new_df), batch_size=batch_size, shuffle=False, collate_fn=hf_collate
)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                enc = image_processor(images=images, return_tensors="pt").pixel_values.to(device)
                outputs = model(enc, labels=labels.to(device))
