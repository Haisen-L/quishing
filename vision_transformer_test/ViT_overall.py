...
preds, labs = evaluate(model, DataLoader(QRCodeDataset(test_df, preprocess), batch_size=1024), device)
...
cm_path = os.path.join(CM_DIR, f"{FOLDER_NAME}_vit_huge_overall_test.png")
...
#########################
all_preds, _ = preds, labs  # reuse overall preds and labs if needed
