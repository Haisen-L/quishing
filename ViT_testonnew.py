...
BATCH_SIZE = 32          # mini‑batch size per GPU (fits ViT‑H memory)
GRAD_ACCUM = 16          # accumulate → effective batch = 512           # gradient accumulation → effective batch = 512
LR = 4e-4                # learning rate scaled linearly for effective batch (4× baseline)                # learning rate tuned for effective batch
...
#########################
# Model Definition (Using ViT-H/14)
#########################
model = timm.create_model(
    "vit_huge_patch14_224", pretrained=True, num_classes=2
).to(device)
model.eval()
...
# Training Function
#########################
def train(model, train_loader, val_loader, optimizer, criterion, device, epochs, grad_accum):
    """Train with gradient accumulation.
    grad_accum: number of mini‑batches to accumulate before optimizer.step()"""
    model.train()
    for epoch in range(epochs):
        ...
        for step, (images, labels) in enumerate(train_loader, 1):
            ...
            loss = criterion(outputs, labels) / grad_accum  # scale loss
            loss.backward()
            total_train_loss += loss.item() * grad_accum  # unscale for logging
...
# Run training
train(model, train_loader, val_loader, optimizer, criterion, device, epochs=5, grad_accum=GRAD_ACCUM)
#########################
# Evaluation Function
#########################
